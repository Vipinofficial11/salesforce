  ...
  public static void queryObject(String Id,String objectName) {
    ...
    try {
      ...
      if (statusCode == 200) {
        ...
        System.out.println(objectResponseList);
  ...
  public static void deletePushTopic(String pushTopicName) {
    try {
      PartnerConnection partnerConnection = new PartnerConnection(
        Authenticator.createConnectorConfig(new AuthenticatorCredentials(
          System.getenv("SALESFORCE_USERNAME"), System.getenv("SALESFORCE_PASSWORD") +
          System.getenv("SALESFORCE_SECURITY_TOKEN"),
          System.getenv("SALESFORCE_CONSUMER_KEY"), System.getenv("SALESFORCE_CONSUMER_SECRET"),
  ...
  public static void deleteId(String Id, String objectName) {
    ...
    try {
      ...
      if (statusCode == 204) {
        ...
        System.out.println("Failed to delete Id. Status code: " + statusCode);
      }
    } catch (IOException ioException) {
      logger.info("Error in establishing connection to Salesforce: " + ioException);
    }
  }
  public static void queryObjectByOther(String fieldName, String fieldValue, String objectName) {
    getAccessToken();
    HttpClient httpClient = HttpClientBuilder.create().build();
    String baseUri = loginInstanceUrl + REST_ENDPOINT + API_VERSION;
    String uri = baseUri + "/sobjects/" + objectName + "/query?q=" + fieldName + "='" + fieldValue + "'";
    HttpGet httpGet = new HttpGet(uri);
    Header oauthHeader = new BasicHeader("Authorization", "Bearer " + loginAccessToken);
    httpGet.addHeader(oauthHeader);
    httpGet.addHeader(prettyPrintHeader);
